...
const page = () => {
    ...
    const works = [
        ...
        },
        {
            ...
            li1_1: "Create or join group video calls to interact with multiple friends at once.",
            li2_2: 'Organize virtual parties and events, invite friends, and have fun together.'
        },
        {
            ...
            li1_1: "Update your preferences anytime to refine the type of content you see and the people you connect with."
        },
        {
            title: 'Notifications',
    ...
    return (
        <div className='px-4'>
            ...
            <div className='text-center'>
                <h1 className='text-sm md:text-base font-medium'><span className='text-lg md:text-2xl text-[#F45044] font-semibold'> Welcome to Connectly!</span>
                    <br /> Here's a quick guide on how to make the most out of your experience:</h1>
            </div>

            {works.map((w, i) => {
                return (
                    <>
                        <div key={i} className='px-3 py-5 my-5 hover:shadow-xl hover:shadow-[#F45044]'>
                            <h3 className='text-sm md:text-xl font-semibold text-[#F45044] mb-2 md:mb-4 flex items-center gap-1'><Image src={stars} className='w-5 h-5' /> {w.title}</h3>
                            <ul className='pl-5'>
                                <li className='mb-1 md:mb-2 font-medium text-[0.8rem] md:text-base'> <input type="checkbox" checked/> {w.subTitle}</li>
                                <ol className='pl-5'>
                                    <li className='before:content-["-"] before:pr-2 before:text-[#00AEF0] text-xs md:text-base'>{w.li1}</li>
                                    <li className='before:content-["-"] before:pr-2 before:text-[#00AEF0] text-xs md:text-base'>{w.li2}</li>
                                    {w.li3 ? (<li className='before:content-["-"] before:pr-2 before:text-[#00AEF0] text-xs md:text-base'>{w.li3}</li>) : (null)}

                                </ol>
                                {w.sec ? (
                                    <>
                                        <li className='mb-1 md:mb-2 font-medium text-[0.8rem] md:text-base'> <input type="checkbox" checked/> {w.subTitle2}</li>
                                        <ol className='pl-5'>
                                            <li className='before:content-["-"] before:pr-2 before:text-[#00AEF0] text-xs md:text-base'>{w.li1_1}</li>
                                            {w.li2_2 ? (<li className='before:content-["-"] before:pr-2 before:text-[#00AEF0] text-xs md:text-base'>{w.li2_2}</li>) : (null)}
                                            {w.li3_3 ? (<li className='before:content-["-"] before:pr-2 before:text-[#00AEF0] text-xs md:text-base'>{w.li3_3}</li>) : (null)}
                                        </ol>
                                    </>
                                ) : (null)}
                            </ul>
